        ...
        static void	checkConformity(std::string const & infile, std::pair<int, std::string const> line) {
            ...
            for (unsigned int i = 0; i != line.second.length(); i++) {
                switch (line.second[i]) {
                    ...
                    case '>':
                        if (chev == false)
                            throw (FormatError("incorrect xml syntax", getEmplacement(infile, line), "'<' is needed before closing one"));
                        if (line.second[i - 1] == ' ')
                            throw (FormatError("incorrect xml syntax", getEmplacement(infile, line), "invalid space caracter before '>'"));
                        chev = false;
                        break;
                    default:
                        car = true;
            ...
            if (chev == true)
                throw (FormatError("incorrect xml syntax", getEmplacement(infile, line), "missing '>' at the end of expression"));
            if (quotes == true)
        ...
        Xml::Xml(std::string const & infile) : ADataFormat(XML, infile) {
            ...
            for (std::list< std::pair<int, std::string> >::iterator it2 = epur.begin(); it2 != epur.end(); ++it2) {
                if (testIfBalise(it2->second) == true) {
                    ...
                    if (name.empty() == true)
                        throw (FormatError("invalid xml syntax", getEmplacement(infile, *it2), "balise name can't be empty"));
                    Balise *bal = new Balise(name, eS);
                    this->_balises.push_back(bal);
                }
                else {
                    if (this->_balises.back()->getState() == CLOSING)
                        throw (FormatError("invalid content declaration", getEmplacement(infile, *it2), "ambigous statement \"" + Parser::epurString(it2->second) + "\""));
            ...
            for (std::list<Balise *>::iterator it2 = this->_balises.begin(); it2 != this->_balises.end(); ++it2) {
                if ((*it2)->getAssociated() == 0) {
                    if ((*it2)->getState() == OPENING)
                        throw (FormatError("invalid xml syntax", infile, "balise \"<" + (*it2)->getName() + ">\" has no related closing one"));
                    else
                        throw (FormatError("invalid xml syntax", infile, "balise \"</" + (*it2)->getName() + ">\" has no related opening one"));
                }
            }
            // for (std::list<Balise *>::iterator it2 = this->_balises.begin(); it2 != this->_balises.end(); ++it2) {
