//
// MainMenu.cpp for bomberman in /home/fortin_j/tek2/projects/bomberman
//
...
namespace BomberMan
{
  namespace Display
  {
    MainMenu::MainMenu(Core::BomberMan* const core) : _core(core)
    {
      this->_cursor = 0;
      ...
      else if ((move = dynamic_cast<const Event::Move*>(event)))
	{
	  if (move->getDirection() == Event::EventDirection::UP)
	    {
	      if ((this->_cursor - 1) > MainMenu::_nbOnglet)
		this->_cursor = MainMenu::_nbOnglet;
	      else
		this->_cursor--;
	      this->_cursorMove();
	    }
    ...
    MenuEnum::eMenu	MainMenu::getType() const
    {
      return MenuEnum::MAIN;
    }

    void	MainMenu::_cursorMove()
    {
      if (this->_cursor > MainMenu::_nbOnglet)
	this->_cursor = 0;
